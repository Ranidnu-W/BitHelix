...
function RotatingParticleScene() {
  const size = 256 // Reduced for better performance
  ...
  const { particles, originalPositionTexture, particlePositions } = useMemo(() => {
    ...
    const geometry = new THREE.TorusKnotGeometry(1.2, 0.3, 200, 16) // Reduced complexity
  ...
  useFrame((state) => {
    ...
    // Rotate points - this creates the rotating effect
...
export default function FluidBackground() {
  return (
    <div className="absolute left-0 top-0 w-[50vw] h-full opacity-60 pointer-events-none z-5 -translate-x-8">
      <Canvas camera={{ position: [0, 0, 10], fov: 60 }}>
        <Suspense fallback={null}>
          <RotatingParticleScene />
        </Suspense>
      </Canvas>
    </div>
  )
